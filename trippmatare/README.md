...
![Trippmätaren i drift](skarmdump.png)
...
den i helskärm utan flikar, och den fungerar offline.
...
minuter stillastående**. Den här appen gör i stället så här:
...
`npm run test:tripp` kör appen i en riktig webbläsare mot simulerade banor med känd
...
3. Spara — sidan hamnar på `https://<användarnamn>.github.io/background-/trippmatare/`

Lokalt: `npx serve .` och öppna `http://localhost:3000/trippmatare/` (GPS fungerar
på `localhost` utan https).
...
```bash
npm install
npm run test:tripp        # sätt CHROMIUM=/sökväg/till/chrome om den inte hittas
```
